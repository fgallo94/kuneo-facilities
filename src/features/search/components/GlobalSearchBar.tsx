  ...
  return (
    <div className="relative w-full max-w-md">
      <div className="relative">
        <Search className="absolute left-3 top-1/2 h-4 w-4 -trangray-y-1/2 text-gray-400" />
        <Input
          type="text"
        ...
        {query && (
          <button
            onClick={() => setQuery('')}
            className="absolute right-2 top-1/2 -trangray-y-1/2 rounded p-0.5 text-gray-400 hover:text-gray-600"
          >
          ...
          <ul className="max-h-64 overflow-y-auto py-1">
            {results.map((inc) => (
              ...
              >
                ...
                <span className="ml-2 text-xs text-gray-500">{inc.status}</span>
              </li>
            ))}
          </ul>
        </div>
